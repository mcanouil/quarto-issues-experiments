...
When using the `brand` feature with local font files (`source: file`) in a Reveal.js format, the `@font-face` declarations generated by the brand system use paths relative to the project root (e.g., `_extensions/repro/fonts/FiraCode-Regular.ttf`).
...
```text
{output}_files/libs/revealjs/dist/theme/_extensions/repro/fonts/FiraCode-Regular.ttf
```
...
```text
[WARNING] Could not fetch resource example_files/libs/revealjs/dist/theme/_extensions/repro/fonts/FiraCode-Regular.ttf
